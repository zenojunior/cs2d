...
function drawCoachGrenade(g: CoachGrenade) {
  ...
  const gd = interaction.grenadeDrag
  const pos = gd && gd.id === g.id ? gd : g
  const { x, y } = w2s(pos.x, pos.y)
...
// Bubble card rects drawn this frame (comment mode), for click hit-testing. The
// render loop fills it; the interaction layer reads it back for hit-testing.
const commentHitboxes = ref<{ id: string; x: number; y: number; w: number; h: number }[]>([])
...
function draw() {
  ...
  commentHitboxes.value = []
  // Snapshot the live drag/hover state for this frame (getters read once).
  const { resizing, playerDrag, coachPath, coachStart, coachCur, hoverMouse, areaStart, areaCur } =
    interaction
  activeComments.forEach((c) => {
    ...
    if (c.anchor.kind === 'area') {
      ...
      if (c.textInside) {
        ...
        if (labelRect) {
          commentHitboxes.value.push({ id: c.id, ...labelRect })
    ...
    if (rect) {
      ...
      commentHitboxes.value.push({ id: c.id, ...rect })
...
// All pointer interaction (wheel/pan, comment placement/hover/resize, coach board)
// + hit-testing live in their own module. It owns the transient drag/hover state
// the render loop reads back through `interaction`, and resolves player poses
// (override/drag-aware) that drawPlayer consumes via `coachPosOf`.
const {
  coachPosOf,
  hoveredCommentId,
  resizeCursor,
